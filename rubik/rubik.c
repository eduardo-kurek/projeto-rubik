...
#pragma region AUXILIARES
    ...
    */
    ...
    Movimento* movimento_aleatorio(Rubik* rubik);
...
#pragma region IMPLEMENTAÇÕES
    ...
    void rubik_embaralhar(Rubik* rubik, short int qt){
        if(rubik->ultimo == NULL)
            printf("Nenhum movimento realizado no cubo ainda \n");
        else{
            printf("Ultimo movimento: %d \n", rubik->ultimo->indice);
            if(movimento_valido(rubik, L))
                printf("Movimento L é valido");
            else
                printf("Movimento L é invalido");
        }
    }
